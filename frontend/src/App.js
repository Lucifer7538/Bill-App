...
export default function App() {
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      if (!e || typeof e.key !== 'string') return; // SAFEGUARD: Stops unmapped virtual keyboard keys from crashing the app
      ...
      // Check User Configured Shortcuts (Safely cast to string to prevent numerical crashes)
      const matchedSc = (settings.shortcuts || []).find(sc => 
        sc && sc.key && String(e.key).toLowerCase() === String(sc.key).toLowerCase() && 
  ...
  return (
    <div className="billing-app">
      {/* PERFECTED CSS: Safely injected to prevent React Hydration Crashes */}
          ...
          <div className="control-card">
              ...
              <div style={{ backgroundColor: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                ...
                {paymentMethod === "Split" && (
                  <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                    <Input inputMode="decimal" value={splitCash} onChange={(e) => { setSplitCash(e.target.value); markDirty(); }} placeholder="Cash Received ₹" />
            ...
            {(txType === "booking" || txType === "service") && (
              <div style={{ backgroundColor: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                ...
                <Input inputMode="decimal" placeholder="Advance Received ₹" value={advanceAmount} onChange={e => {setAdvanceAmount(e.target.value); markDirty();}} style={{ marginBottom: '10px' }} />
                ...
                {advanceMethod === "Split" && (
                  <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                    <Input inputMode="decimal" value={advanceSplitCash} onChange={(e) => { setAdvanceSplitCash(e.target.value); markDirty(); }} placeholder="Cash Portion ₹" />
                ...
                {balanceMethod === "Split" && (
                  <div style={{ display: "flex", gap: "10px", marginTop: "10px" }}>
                    <Input inputMode="decimal" value={balanceSplitCash} onChange={(e) => { setBalanceSplitCash(e.target.value); markDirty(); }} placeholder="Cash Portion ₹" />
