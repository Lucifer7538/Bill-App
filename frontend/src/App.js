      ...
      <main className="main-layout" style={isPrinting ? { height: "auto", overflow: "visible", display: "block" } : { flex: 1, display: "flex", flexDirection: isMobileSplit ? "column" : "row", overflowY: isMobileSplit ? "auto" : "hidden", overflowX: "hidden", backgroundColor: "#f1f5f9", minHeight: 0, paddingBottom: isMobileSplit ? "40px" : "0" }}>
        ...
        <aside className="controls no-print" style={{ flex: isMobileSplit ? "none" : "2", overflowY: isMobileSplit ? "visible" : "auto", overflowX: "hidden", padding: "20px", backgroundColor: "white", borderLeft: isMobileSplit ? "none" : "1px solid #cbd5e1", borderTop: isMobileSplit ? "1px solid #cbd5e1" : "none", height: isMobileSplit ? "max-content" : "100%" }}>
          ...
          <div className="control-card">
            ...
            {(items || []).map((item) => (
              <div key={item.id} className="item-row-editor">
                ...
                {/* --- NEW SPELL CHECKER & AUTO SUGGEST UI INJECTED HERE --- */}
                <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
                  <Input 
                    ...
                    style={{ flex: 1, paddingRight: spellCheckResults[item.id] ? "30px" : "10px" }}
                  />
                  
                  {/* Spell Checker Indicator Dot */}
                  {spellCheckResults[item.id] && item.description.trim() !== "" && (
                    <div
                      onClick={() => {
                        if (!spellCheckResults[item.id].valid) setActiveSpellRow(activeSpellRow === item.id ? null : item.id);
                      }}
                      style={{
                        position: "absolute", right: "10px", width: "12px", height: "12px", borderRadius: "50%",
                        cursor: spellCheckResults[item.id].valid ? "default" : "pointer",
                        backgroundColor: spellCheckResults[item.id].valid ? "#22c55e" : "#ef4444",
                        boxShadow: spellCheckResults[item.id].valid ? "none" : "0 0 5px #ef4444"
                      }}
                      title={spellCheckResults[item.id].valid ? "Spelling OK" : "Typo detected? Click to fix."}
                    />
                  )}

                  {/* Auto-Suggestion Dropdown (Inventory Scan) */}
                  {activeSuggestionRow === item.id && item.description.trim().length >= 1 && (
                    <div style={{ position: "absolute", top: "100%", left: 0, right: 0, backgroundColor: "white", border: "1px solid #cbd5e1", zIndex: 50, maxHeight: "150px", overflowY: "auto", boxShadow: "0 4px 6px rgba(0,0,0,0.1)", borderRadius: "6px", marginTop: "4px" }}>
                      {inventoryMemory.filter(opt => opt.toLowerCase().includes(item.description.toLowerCase()) && opt.toLowerCase() !== item.description.toLowerCase()).map((opt, i) => (
                        <div key={i} style={{ padding: "10px", cursor: "pointer", borderBottom: "1px solid #f1f5f9", fontWeight: "bold", color: "#0f172a" }} onMouseDown={() => { updateItem(item.id, "description", opt); setActiveSuggestionRow(null); }}>
                          {opt}
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Spell Check Suggestions "Do you mean?" Box */}
                  {activeSpellRow === item.id && !spellCheckResults[item.id].valid && (
                    <div style={{ position: "absolute", top: "100%", right: 0, backgroundColor: "white", border: "1.5px solid #ef4444", zIndex: 60, width: "max-content", minWidth: "180px", boxShadow: "0 10px 15px rgba(0,0,0,0.2)", borderRadius: "8px", padding: "10px", marginTop: "4px" }}>
                      <p style={{ margin: "0 0 8px 0", fontSize: "0.85rem", color: "#475569", fontWeight: "bold" }}>Do you mean?</p>
                      {spellCheckResults[item.id].suggestions.map((sug, i) => (
                        <div key={i} style={{ padding: "8px 12px", cursor: "pointer", backgroundColor: "#fef2f2", color: "#b91c1c", marginBottom: "4px", borderRadius: "6px", fontWeight: "bold", fontSize: "1.1rem" }} onMouseDown={() => applySpellCorrection(item.id, item.description, spellCheckResults[item.id].targetWord, sug)}>
                          {sug}
                        </div>
                      ))}
                    </div>
          ...
          <div className="control-card action-grid">
            ...
            {/* --- NEW ANALYTICS BUTTON ADDED HERE --- */}
            <Button onClick={() => setShowAnalytics(true)} style={{ backgroundColor: "#0284c7", color: "white" }}><BarChart3 size={18} style={{ marginRight: "5px" }}/> 📊 Analyze Business</Button>
            ...
            {/* --- NEW INVENTORY BUTTON ADDED HERE --- */}
            <Button onClick={() => toast.info("Inventory Management Module Coming Soon!")} variant="outline" style={{ borderColor: "#64748b", color: "#64748b" }}>Inventory</Button>
          </div>

        </aside>
      </main>

      {/* --- NEW BUSINESS ANALYTICS DASHBOARD DRAWER --- */}
      {showAnalytics && (
        <section className="side-drawer no-print" style={{ position: "fixed", top: 0, bottom: 0, right: 0, width: "100vw", maxWidth: "800px", backgroundColor: "white", zIndex: 100, boxShadow: "-5px 0 25px rgba(0,0,0,0.2)", overflowY: "auto" }}>
          <div className="drawer-header" style={{ backgroundColor: "#f0f9ff", borderBottom: "2px solid #bae6fd", padding: "20px", position: "sticky", top: 0, zIndex: 10 }}>
            <h3 style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", margin: 0, color: "#0369a1" }}><BarChart3 /> Business Analytics Dashboard</h3>
            <Button type="button" variant="outline" className="drawer-back-btn" onClick={() => setShowAnalytics(false)} style={{ marginTop: "10px", borderColor: "#0284c7", color: "#0284c7" }}><ArrowLeft className="drawer-back-icon" style={{ marginRight: "5px" }} /><span>Close Dashboard</span></Button>
          </div>

          <div id="analytics-print-root" style={{ padding: "20px", backgroundColor: "#f8fafc", minHeight: "100%" }}>
            
            {/* Header for PDF Generation */}
            <div className="print-only" style={{ textAlign: "center", marginBottom: "20px" }}>
              <h2 style={{ margin: 0 }}>{settings.shop_name} - Business Analytics</h2>
              <p style={{ margin: "5px 0 0 0", color: "#64748b" }}>Generated on: {today()}</p>
            </div>

            <div className="no-print" style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "25px", padding: "15px", backgroundColor: "white", borderRadius: "8px", border: "1px solid #cbd5e1" }}>
              <div style={{ flex: 1, minWidth: "200px" }}>
                <label className="select-label">Timeframe Filter</label>
                <select value={analyticsFilter} onChange={(e) => setAnalyticsFilter(e.target.value)} className="native-select" style={{ backgroundColor: "#f1f5f9" }}>
                  <option value="CURRENT_DAY">Current Day</option>
                  <option value="CURRENT_WEEK">Current Week</option>
                  <option value="CURRENT_HALF_MONTH">Current Half Month (1st-15th / 16th-End)</option>
                  <option value="THIS_MONTH">Current Month</option>
                  <option value="LAST_MONTH">Previous Month</option>
                  <option value="PREVIOUS_3_MONTHS">Previous 3 Months</option>
                  <option value="PREVIOUS_6_MONTHS">Previous 6 Months</option>
                  <option value="PREVIOUS_YEAR">Previous Year</option>
                  <option value="ALL">Till Opening (All Time)</option>
                </select>
              </div>
              <div style={{ display: "flex", alignItems: "flex-end" }}>
                <Button onClick={() => downloadPdf("analytics-print-root", `Jalaram_Analytics_${analyticsFilter}`)} style={{ backgroundColor: "#0f172a", color: "white" }}>
                  <Download size={16} style={{ marginRight: "8px" }}/> Download PDF
                </Button>
              </div>
            </div>

            <div style={{ padding: "20px", backgroundColor: "white", borderRadius: "12px", border: "1px solid #e2e8f0", boxShadow: "0 4px 6px rgba(0,0,0,0.05)" }}>
               <h4 style={{ margin: "0 0 20px 0", fontSize: "1.2rem", color: "#0f172a", borderBottom: "1px dashed #cbd5e1", paddingBottom: "10px" }}>Revenue Overview: {analyticsFilter.replace(/_/g, ' ')}</h4>
               
               {/* Analytics Data Calculations based on filteredRecentBills */}
               {(() => {
                 let invTotal = 0; let estTotal = 0; let invCount = 0; let estCount = 0;
                 (filteredRecentBills || []).forEach(b => {
                    const amt = b.totals?.grand_total || 0;
                    if (b.mode === 'invoice') { invTotal += amt; invCount++; }
                    else { estTotal += amt; estCount++; }
                 });
                 const maxVal = Math.max(invTotal, estTotal, 1);
                 const invPct = (invTotal / maxVal) * 100;
                 const estPct = (estTotal / maxVal) * 100;

                 return (
                   <div>
                     <div style={{ display: "flex", flexWrap: "wrap", gap: "15px", marginBottom: "30px" }}>
                       <div style={{ flex: "1 1 200px", padding: "20px", backgroundColor: "#fef2f2", borderRadius: "10px", border: "1px solid #fecaca", textAlign: "center" }}>
                          <p style={{ margin: "0 0 5px 0", color: "#991b1b", fontWeight: "bold" }}>TAX INVOICE REVENUE</p>
                          <h2 style={{ margin: 0, color: "#dc2626", fontSize: "1.8rem" }}>₹{money(invTotal)}</h2>
                          <p style={{ margin: "5px 0 0 0", color: "#b91c1c", fontSize: "0.85rem" }}>{invCount} Bills Generated</p>
                       </div>
                       <div style={{ flex: "1 1 200px", padding: "20px", backgroundColor: "#eff6ff", borderRadius: "10px", border: "1px solid #bfdbfe", textAlign: "center" }}>
                          <p style={{ margin: "0 0 5px 0", color: "#1e40af", fontWeight: "bold" }}>ESTIMATE REVENUE</p>
                          <h2 style={{ margin: 0, color: "#2563eb", fontSize: "1.8rem" }}>₹{money(estTotal)}</h2>
                          <p style={{ margin: "5px 0 0 0", color: "#1d4ed8", fontSize: "0.85rem" }}>{estCount} Bills Generated</p>
                       </div>
                       <div style={{ flex: "1 1 100%", padding: "20px", backgroundColor: "#f0fdf4", borderRadius: "10px", border: "1px solid #bbf7d0", textAlign: "center" }}>
                          <p style={{ margin: "0 0 5px 0", color: "#166534", fontWeight: "bold" }}>TOTAL COMBINED REVENUE</p>
                          <h2 style={{ margin: 0, color: "#16a34a", fontSize: "2rem" }}>₹{money(invTotal + estTotal)}</h2>
                       </div>
                     </div>

                     <h4 style={{ margin: "0 0 15px 0", color: "#475569" }}>Visual Comparison</h4>
                     <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                        <div>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", fontWeight: "bold", marginBottom: "5px", color: "#dc2626" }}>
                            <span>Invoices</span><span>₹{money(invTotal)}</span>
                          </div>
                          <div style={{ width: "100%", height: "20px", backgroundColor: "#f1f5f9", borderRadius: "10px", overflow: "hidden" }}>
                            <div style={{ width: `${invPct}%`, height: "100%", backgroundColor: "#dc2626", transition: "width 1s ease-in-out" }}></div>
                          </div>
                        </div>
                        <div>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", fontWeight: "bold", marginBottom: "5px", color: "#2563eb" }}>
                            <span>Estimates</span><span>₹{money(estTotal)}</span>
                          </div>
                          <div style={{ width: "100%", height: "20px", backgroundColor: "#f1f5f9", borderRadius: "10px", overflow: "hidden" }}>
                            <div style={{ width: `${estPct}%`, height: "100%", backgroundColor: "#2563eb", transition: "width 1s ease-in-out" }}></div>
                          </div>
                        </div>
                     </div>
                   </div>
