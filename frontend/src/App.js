...
const defaultSettings = {
  ...
  branches: [
    { 
      ...
      location_url: "", map_url: "", whatsapp_url: "", instagram_url: "", about_url: "", 
      invoice_upi_id: "eazypay.0000048595@icici", estimate_upi_id: "7538977527@ybl", gstin: "21AAUFJ1925F1ZH", 
    ...
    <div style={{ marginTop: "25px", borderTop: "1px dashed #e2e8f0", paddingTop: "20px" }}>
      <div className="no-print" data-html2canvas-ignore="true">
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '15px' }}>
          {branch.whatsapp_url && (
            <a href={branch.whatsapp_url} target="_blank" rel="noopener noreferrer" style={{ flex: '1 1 120px', padding: "10px", backgroundColor: "#25D366", color: "white", textAlign: "center", textDecoration: "none", fontWeight: "bold", borderRadius: "8px" }}>💬 WhatsApp</a>
          )}
          {branch.instagram_url && (
            <a href={branch.instagram_url} target="_blank" rel="noopener noreferrer" style={{ flex: '1 1 120px', padding: "10px", background: "linear-gradient(45deg, #f09433 0%, #bc1888 100%)", color: "white", textAlign: "center", textDecoration: "none", fontWeight: "bold", borderRadius: "8px" }}>📸 Instagram</a>
          )}
          {branch.map_url && (
            <a href={branch.map_url} target="_blank" rel="noopener noreferrer" style={{ flex: '1 1 120px', padding: "10px", backgroundColor: "#facc15", color: "#854d0e", textAlign: "center", textDecoration: "none", fontWeight: "bold", borderRadius: "8px" }}>⭐ Feedback</a>
          )}
        </div>
      </div>
...
const DesignSettingRow = ({ title, fieldPrefix, settings, setSettings }) => (
  <div style={{ padding: "12px", border: "1px solid #e2e8f0", borderRadius: "8px", marginBottom: "15px", backgroundColor: "#f8fafc", width: "100%", boxSizing: "border-box" }}>
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center", width: "100%" }}>
      ...
      <select value={settings[`${fieldPrefix}_font`] || "sans-serif"} onChange={(e) => setSettings((prev) => ({ ...prev, [`${fieldPrefix}_font`]: e.target.value }))} style={{ flex: "1 1 120px", height: "35px", border: "1px solid #ccc", borderRadius: "6px", fontSize: "0.85rem", padding: "0 5px", minWidth: "120px" }} title="Font Style">
        <FontSelectOptions customFonts={settings.custom_fonts || []} />
      </select>
      <select value={settings[`${fieldPrefix}_align`] || "center"} onChange={(e) => setSettings((prev) => ({ ...prev, [`${fieldPrefix}_align`]: e.target.value }))} style={{ flex: "1 1 80px", height: "35px", border: "1px solid #ccc", borderRadius: "6px", fontSize: "0.85rem", padding: "0 5px", minWidth: "80px" }} title="Alignment">
...
// === PART 4 START ===
  ...
  // CRASH GUARDED RECENT BILLS FETCH
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try { const response = await axios.get(`${API}/customers/suggest`, { headers: authHeaders, params: { query } }); setSuggestions(Array.isArray(response.data) ? response.data : []); } catch { setSuggestions([]); }
    }, 250);
    return () => clearTimeout(timer);
...
// === PART 5 START ===
  ...
  const handleQuickPaymentToggle = async (bill) => {
    ...
    try { 
      ...
      setRecentBillsList(prev => Array.isArray(prev) ? prev.map(b => b.document_number === bill.document_number ? { ...b, is_payment_done: newStatus } : b) : []); 
      await loadSettings(); 
    } 
  ...
  const saveBill = async () => {
    ...
    setSavedDescriptions(prev => {
      const updated = Array.from(new Set([...(Array.isArray(prev) ? prev : []), ...newItems]));
...
// === PART 4 END ===
// === PART 5 START ===
  // CRASH GUARD FIX: WhatsApp phone number casting
  const shareWhatsApp = () => { 
    ...
    // Safety check: force phone number to string before .replace
              ...
              <div key={item.id} className="item-row-editor" style={{ overflow: "visible" }}>
                ...
                <div style={{ position: "relative" }}>
                  <Input 
                    ...
                    style={{ width: "100%", paddingRight: hasTypo ? "110px" : "10px" }}
                  />
                  {hasTypo && (
                    <button 
                      type="button"
                      onMouseDown={(e) => {
                        e.preventDefault();
                        updateItem(item.id, 'description', correctedText);
                      }}
                      style={{
                        position: 'absolute',
                        right: '5px',
