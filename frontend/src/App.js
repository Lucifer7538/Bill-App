...
const defaultSettings = {
  ...
  address_color: "#475569", address_size: 14, address_font: "sans-serif", address_align: "center", phone_color: "#475569", phone_size: 13, phone_font: "sans-serif", phone_align: "center", email_color: "#475569", email_size: 13, email_font: "sans-serif", email_align: "center",
  silver_rate_per_gram: 240, making_charge_per_gram: 15, flat_mc_below_5g: 150, default_hsn: "7113", loyalty_points_per_gram: 1, loyalty_point_value_rs: 1,
...
@media print { .print-only { position: static !important; width: 100% !important; height: auto !important; opacity: 1 !important; visibility: visible !important; overflow: visible !important; display: flex !important; } .no-print { display: none !important; } }
`;
// === PART 1 END ===
// === PART 2 START ===
...
// === PART 3 START ===
  ...
  useEffect(() => {
      const handleResize = () => setViewportWidth(window.innerWidth); window.addEventListener("resize", handleResize); 
      const handleBeforePrint = () => setIsPrinting(true); const handleAfterPrint = () => setIsPrinting(false);
      window.addEventListener("beforeprint", handleBeforePrint); window.addEventListener("afterprint", handleAfterPrint);
      return () => { window.removeEventListener("resize", handleResize); window.removeEventListener("beforeprint", handleBeforePrint); window.removeEventListener("afterprint", handleAfterPrint); };
  }, []);
  ...
  useEffect(() => {
    ...
    if (params.get("view")) {
      ...
      const fetchPublicBill = async () => {
        try { const res = await axios.get(`${API}/bills/public/${params.get("view")}`); setPublicBill(res.data.bill); const sData = { ...defaultSettings, ...res.data.settings }; if (!sData.branches) sData.branches = defaultSettings.branches; if (sData.custom_fonts) sData.custom_fonts.forEach(f => registerFont(f.name, f.dataUrl)); setPublicSettings(sData); } catch (err) { setPublicBill("NOT_FOUND"); } finally { setPublicLoading(false); }
      }; fetchPublicBill();
    }
  }, []);

  useEffect(() => { const handleEsc = (event) => { if (event.key === "Escape") { setShowSettings(false); setShowAbout(false); setShowRecentBills(false); setShowLedger(false); setShowFeedbackModal(false); } }; window.addEventListener("keydown", handleEsc); return () => window.removeEventListener("keydown", handleEsc); }, [showSettings, showAbout, showRecentBills, showLedger, showFeedbackModal]);
  ...
  const filteredRecentBills = useMemo(() => {
    const list = Array.isArray(recentBillsList) ? recentBillsList : [];
...
// === PART 4 START ===
  ...
  const computed = useMemo(() => {
    ...
    const mapped = (items || []).map((item, index) => {
      const weight = num(item.weight); totalWeight += weight; const quantity = Math.max(num(item.quantity || 1), 1);
      ...
      const totalItemCost = (weight * silverRate) + mcAmount; const formulaAmount = mode === "estimate" ? totalItemCost * quantity : totalItemCost;
      const amount = item.amount_override !== "" ? num(item.amount_override) : formulaAmount;
      const rateForPrint = weight > 0 ? (amount / (mode === "estimate" ? quantity : 1)) / weight : 0; const { rupees, paise } = splitAmount(amount);
    ...
    const cgst = mode === "invoice" ? taxable * 0.015 : 0; const sgst = mode === "invoice" ? taxable * 0.015 : 0; const igst = 0; const gstApplied = mode === "invoice" ? cgst + sgst + igst : 0;
    const mdr = paymentMethod === "Card" ? (taxable + gstApplied) * 0.02 : 0;
    const bonusPointsVal = num(bonusPoints); const earnedPoints = Math.floor(totalWeight * ptPerGram) + bonusPointsVal;
    const appliedRedeemedPoints = num(redeemedPoints); const appliedRedeemedValue = appliedRedeemedPoints * rsPerPt; const appliedCreditVal = num(appliedCredit); const savedCreditVal = num(savedCredit);
  ...
  const publicComputed = useMemo(() => {
    ...
    const ptPerGram = num(publicSettings.loyalty_points_per_gram !== undefined ? publicSettings.loyalty_points_per_gram : 1); const rsPerPt = num(publicSettings.loyalty_point_value_rs !== undefined ? publicSettings.loyalty_point_value_rs : 1);
    let totalWeight = 0;
    const mapped = (publicBill.items || []).map((item, index) => {
      ...
      const totalItemCost = (weight * silverRate) + mcAmount; const formulaAmount = publicBill.mode === "estimate" ? totalItemCost * quantity : totalItemCost;
      ...
      const { rupees, paise } = splitAmount(amount); const rateForPrint = weight > 0 ? (amount / (publicBill.mode === "estimate" ? quantity : 1)) / weight : 0;
      return { ...item, sl_no: item.sl_no || (index + 1), rate: rateForPrint, amount, rupees, paise, weight, quantity };
    });
    const subtotal = mapped.reduce((sum, row) => sum + row.amount, 0); const taxable = subtotal;
    const cgst = publicBill.mode === "invoice" ? taxable * 0.015 : 0; const sgst = publicBill.mode === "invoice" ? taxable * 0.015 : 0; const igst = 0; const gstApplied = publicBill.mode === "invoice" ? cgst + sgst + igst : 0;
    ...
    const bonusPointsVal = num(publicBill.bonus_points || 0); const earnedPoints = publicBill.earned_points !== undefined ? num(publicBill.earned_points) : (Math.floor(totalWeight * ptPerGram) + bonusPointsVal);
    const redeemedPoints = num(publicBill.redeemed_points || 0); const redeemedValue = redeemedPoints * rsPerPt; const appliedCreditVal = num(publicBill.applied_credit || 0); const savedCreditVal = num(publicBill.saved_credit || 0);
    const baseTotal = taxable + gstApplied + mdr - discount - exchange - redeemedValue - appliedCreditVal + savedCreditVal; 
    const autoRound = Math.round(baseTotal) - baseTotal; const roundOff = publicBill.round_off !== undefined && publicBill.round_off !== null ? num(publicBill.round_off) : (publicBill.totals?.round_off !== undefined && publicBill.totals?.round_off !== null ? num(publicBill.totals?.round_off) : autoRound);
  ...
  const getUpiAmount = () => {
      if (txType === "sale") { if (isPaymentDone) return 0; if (paymentMethod === "UPI") return computed.grandTotal; if (paymentMethod === "Split") return Math.max(0, computed.grandTotal - num(splitCash)); return 0; }
      if (txType === "booking" || txType === "service") { if (!isAdvancePaid) { if (advanceMethod === "UPI") return num(advanceAmount); if (advanceMethod === "Split") return Math.max(0, num(advanceAmount) - num(advanceSplitCash)); return 0; } if (!isBalancePaid) { const bal = Math.max(0, computed.grandTotal - num(advanceAmount)); if (balanceMethod === "UPI") return bal; if (balanceMethod === "Split") return Math.max(0, bal - num(balanceSplitCash)); return 0; } }
      return 0;
  };
...
// === PART 5 START ===
  ...
  if (isPublicView) {
    ...
    if (!isPaid) {
      if (isSale) { if (publicBill.payment_method === "UPI") publicUpiAmountToPay = publicComputed.grandTotal; else if (publicBill.payment_method === "Split") publicUpiAmountToPay = Math.max(0, publicComputed.grandTotal - num(publicBill.split_cash)); } 
      else { if (!publicBill.is_advance_paid) { if (publicBill.advance_method === "UPI") publicUpiAmountToPay = num(publicBill.advance_amount) || publicComputed.grandTotal; else if (publicBill.advance_method === "Split") publicUpiAmountToPay = Math.max(0, num(publicBill.advance_amount) - num(publicBill.advance_split_cash)); } else if (!publicBill.is_balance_paid) { const bal = Math.max(0, publicComputed.grandTotal - num(publicBill.advance_amount)); if (publicBill.balance_method === "UPI") publicUpiAmountToPay = bal; else if (publicBill.balance_method === "Split") publicUpiAmountToPay = Math.max(0, bal - num(publicBill.balance_split_cash)); } }
    }
        ...
        <section id="public-bill-root" className="bill-sheet" style={{ "--print-scale-factor": 1, position: 'relative', zIndex: 1 }}>
          ...
          <div className="sheet-bottom-stack">
            <div className="totals">
              ...
              {isSale ? (<div className="totals-row" style={{ color: isPaid ? "#16a34a" : "#b45309", marginTop: "10px" }}><span>{isPaid ? "PAID VIA" : "PAYMENT STATUS"}</span><strong>{isPaid ? (publicBill.payment_method === "Split" ? `SPLIT (C:₹${money(publicBill.split_cash)}, U:₹${money(Math.max(0, publicComputed.grandTotal - num(publicBill.split_cash)))})` : (publicBill.payment_method || "CASH").toUpperCase()) : "PENDING"}</strong></div>) : (<><div className="totals-row" style={{ marginTop: "10px", color: publicBill.is_advance_paid ? "#16a34a" : "#b45309" }}><span>ADVANCE {publicBill.is_advance_paid ? "RECEIVED" : "PENDING"} {publicBill.advance_method ? `(${publicBill.advance_method === 'Split' ? `C:₹${money(publicBill.advance_split_cash)}, U:₹${money(Math.max(0, num(publicBill.advance_amount) - num(publicBill.advance_split_cash)))}` : publicBill.advance_method})` : ""}</span><strong>₹{money(publicBill.advance_amount)}</strong></div><div className="totals-row" style={{ color: publicBill.is_balance_paid ? "#16a34a" : "#dc2626" }}><span>BALANCE {publicBill.is_balance_paid ? "RECEIVED" : "DUE"} {publicBill.balance_method ? `(${publicBill.balance_method === 'Split' ? `C:₹${money(publicBill.balance_split_cash)}, U:₹${money(Math.max(0, (publicComputed.grandTotal - num(publicBill.advance_amount)) - num(publicBill.balance_split_cash)))}` : publicBill.balance_method})` : ""}</span><strong>₹{money(Math.max(0, publicComputed.grandTotal - num(publicBill.advance_amount)))}</strong></div></>)}
              {!isPaid && publicUpiAmountToPay > 0 && (<div className="payment-qr-box" data-html2canvas-ignore="true" style={{ textAlign: "center", marginTop: "20px", padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px dashed #cbd5e1" }}><p className="scan-title" style={{ fontWeight: "bold", margin: "0 0 10px 0", color: "#0f172a", fontSize: "1.1rem" }}>Scan Here For Payment (₹{money(publicUpiAmountToPay)})</p><img src={publicDynamicQrUrl} alt="Dynamic payment QR" className="upi-qr" style={{ width: "200px", height: "200px", margin: "0 auto", display: "block" }} crossOrigin="anonymous" /><p className="upi-id" style={{ fontSize: "0.9rem", color: "#64748b", margin: "10px 0 0 0", fontWeight: "bold" }}>UPI: {publicUpiId}</p></div>)}
            </div>
    ...
    if (!isAdminView && !isPublicView) {
      return (
        <div className="login-shell" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f1f5f9', height: '100dvh', padding: '20px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#475569', fontSize: '1.1rem', maxWidth: '400px', lineHeight: '1.6' }}>Please use the secure link provided in your WhatsApp or Email message to securely view your official bill.</p>
        </div>
      );
    }
    return (
  ...
  return (
    <div className="billing-app" style={isPrinting ? { height: "auto", overflow: "visible" } : { display: "flex", flexDirection: "column", height: "100dvh", overflow: "hidden", backgroundColor: "#f1f5f9" }}>
      ...
      <div style={{ position: "absolute", zIndex: -9999, opacity: 0, pointerEvents: "none", top: 0, left: 0, height: 0, overflow: "hidden" }}>
        {(Array.isArray(filteredRecentBills) ? filteredRecentBills : []).map(b => {
           ...
           const printedItems = (b.items || []).map((item, index) => {
             ...
             const { rupees, paise } = splitAmount(amount); return { ...item, sl_no: item.sl_no || (index + 1), rate, amount, rupees, paise };
           });
             ...
             <section key={b.id} id={`bulk-bill-${b.document_number}`} className="bill-sheet" style={{ width: "800px", maxWidth: "800px", margin: 0, "--print-scale-factor": 1 }}>
                ...
                <div className="bill-header">
                  <div className="logo-area">
                    ...
                    <div style={{ width: "100%", textAlign: settings.shop_name_align || "center" }}><h2 className="sheet-shop-title" style={{ fontFamily: settings.shop_name_font || "sans-serif", color: settings.shop_name_color || "#000", fontSize: `${settings.shop_name_size}px`, margin: 0 }}>{settings.shop_name}</h2></div>
                    <div style={{ width: "100%", textAlign: settings.tagline_align || "center" }}><p className="sheet-tagline" style={{ fontFamily: settings.tagline_font || "sans-serif", color: settings.tagline_color || "#475569", fontSize: `${settings.tagline_size}px`, margin: "5px 0" }}>{settings.tagline}</p></div>
                  </div>
                  <div className="contact-area">
                    <div className="contact-address" style={{ fontFamily: settings.address_font || "sans-serif", textAlign: settings.address_align || "center" }}><span style={{ color: settings.address_color || "#475569", fontSize: `${settings.address_size || 14}px` }}>{billBranch.address}</span></div>
                    <div style={{ width: "100%", textAlign: settings.phone_align || "center", fontFamily: settings.phone_font || "sans-serif", fontSize: `${settings.phone_size || 13}px`, marginBottom: "4px" }}>{(settings.phone_numbers || []).join(" | ")}</div>
                ...
                <div className="meta-grid"><p><strong>{b.mode === "invoice" ? "Invoice No" : "Estimate No"}:</strong> {b.document_number}</p><p><strong>Date:</strong> {b.date}</p></div>
                <div className="customer-box"><p><strong>Name:</strong> {b.customer_name || b.customer?.name || "-"}</p><p><strong>Address:</strong> {b.customer_address || b.customer?.address || "-"}</p><p><strong>Phone:</strong> {b.customer_phone || b.customer?.phone || "-"}</p></div>
                ...
                <div className="sheet-bottom-stack">
                  <div className="totals">
                    ...
                    {b.mode === "invoice" ? (<><div className="totals-row"><span>CGST @ 1.5%</span><strong>₹{money(b.totals?.cgst || 0)}</strong></div><div className="totals-row"><span>SGST @ 1.5%</span><strong>₹{money(b.totals?.sgst || 0)}</strong></div><div className="totals-row"><span>IGST @ 0%</span><strong>₹{money(b.totals?.igst || 0)}</strong></div></>) : (<><div className="totals-row"><span>DISCOUNT</span><strong>₹{money(b.totals?.discount || 0)}</strong></div><div className="totals-row"><span>EXCHANGE</span><strong>₹{money(b.totals?.exchange || 0)}</strong></div></>)}
                    ...
                    <div className="totals-row"><span>MDR (Card 2%)</span><strong>₹{money(b.totals?.mdr || 0)}</strong></div>
                    ...
                    {b.tx_type === "sale" || !b.tx_type ? (<div className="totals-row" style={{ color: b.is_payment_done ? "#16a34a" : "#b45309", marginTop: "10px" }}><span>{b.is_payment_done ? "PAID VIA" : "PAYMENT STATUS"}</span><strong>{b.is_payment_done ? (b.payment_method === "Split" ? `SPLIT (C:₹${money(b.split_cash)}, U:₹${money(Math.max(0, (b.totals?.grand_total || 0) - num(b.split_cash)))})` : (b.payment_method || "CASH").toUpperCase()) : "PENDING"}</strong></div>) : (<><div className="totals-row" style={{ marginTop: "10px", color: b.is_advance_paid ? "#16a34a" : "#b45309" }}><span>ADVANCE {b.is_advance_paid ? "RECEIVED" : "PENDING"} {b.advance_method ? `(${b.advance_method === 'Split' ? `C:₹${money(b.advance_split_cash)}, U:₹${money(Math.max(0, num(b.advance_amount) - num(b.advance_split_cash)))}` : b.advance_method})` : ""}</span><strong>₹{money(b.advance_amount)}</strong></div><div className="totals-row" style={{ color: b.is_balance_paid ? "#16a34a" : "#dc2626" }}><span>BALANCE {b.is_balance_paid ? "RECEIVED" : "DUE"} {b.balance_method ? `(${b.balance_method === 'Split' ? `C:₹${money(b.balance_split_cash)}, U:₹${money(Math.max(0, ((b.totals?.grand_total || 0) - num(b.advance_amount)) - num(b.balance_split_cash)))}` : b.balance_method})` : ""}</span><strong>₹{money(Math.max(0, num(b.totals?.grand_total || 0) - num(b.advance_amount)))}</strong></div></>)}
                  </div>
                  {b.earned_points > 0 && (<div style={{ textAlign: "center", marginTop: "15px", padding: "10px", backgroundColor: "#f0fdf4", borderRadius: "8px", border: "1px dashed #22c55e", color: "#166534", fontWeight: "bold", fontSize: "0.9rem" }}>🎉 You earned {b.earned_points} Loyalty Points on this bill!</div>)}
                  <div className="declaration">
                    {b.mode === "invoice" ? (<><p className="section-title">DECLARATION</p><p>We declare that this bill shows the actual price of items and all details are correct.</p><p className="section-title">POLICIES, T&C</p><ul className="policies-list"><li>6 Months of repair and polishing warranty only on silver ornaments.</li><li>You can replace purchased items within 7 days for manufacturing defects.</li></ul></>) : (<><p className="section-title">POLICIES, T&C</p><ul className="policies-list"><li>6 Months of repair and polishing warranty only on silver ornaments.</li><li>You can replace purchased items within 7 days for manufacturing defects.</li></ul></>)}
                  </div>
      ...
      <header className="top-bar no-print" style={{ zIndex: 50, position: "relative", flexShrink: 0, minHeight: "65px", height: "auto", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", flexWrap: "wrap", gap: "10px" }}>
        ...
        <div className="top-actions" style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.85rem", color: iotOnline ? "#4ade80" : "#ef4444", marginRight: "10px", padding: "4px 10px", borderRadius: "20px", border: `1px solid ${iotOnline ? "#4ade80" : "#ef4444"}` }}><Cpu size={14} /><span style={{ width: "8px", height: "8px", borderRadius: "50%", backgroundColor: iotOnline ? "#4ade80" : "#ef4444", boxShadow: iotOnline ? "0 0 8px #4ade80" : "none" }} /><span>Display: {iotOnline ? "Live" : "Off"}</span></div>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.85rem", color: cloudStatus.enabled ? "#4ade80" : "#facc15", marginRight: "10px" }}><span style={{ width: "8px", height: "8px", borderRadius: "50%", backgroundColor: cloudStatus.enabled ? "#4ade80" : "#facc15" }} /><span>Cloud: {cloudStatus.enabled ? "Online" : "Connecting"}</span></div>
        ...
        <section style={isPrinting ? { padding: 0, margin: 0, overflow: "visible" } : { flex: isMobileSplit ? "none" : "3", overflow: isMobileSplit ? "visible" : "auto", padding: "20px", height: isMobileSplit ? "max-content" : "100%" }}>
          <div id="bill-print-root" className="bill-sheet" style={{ "--print-scale-factor": (printScale / 100).toFixed(3), position: 'relative', zIndex: 1, margin: "0 auto" }}>
            ...
            <div className="bill-header">
              ...
              <div className="contact-area">
                <div className="contact-address" style={{ fontFamily: settings.address_font || "sans-serif", display: 'flex', flexDirection: 'column', gap: '3px', marginBottom: '8px', alignItems: settings.address_align === 'left' ? 'flex-start' : settings.address_align === 'right' ? 'flex-end' : 'center', textAlign: settings.address_align || "center" }}><a href={activeBillBranch.location_url && activeBillBranch.location_url !== "#" ? activeBillBranch.location_url : "#"} target="_blank" rel="noopener noreferrer" style={{ color: settings.address_color || "#475569", fontSize: `${settings.address_size || 14}px`, textDecoration: 'none' }}>{activeBillBranch.address}</a></div>
                <div style={{ width: "100%", textAlign: settings.phone_align || "center", fontFamily: settings.phone_font || "sans-serif", fontSize: `${settings.phone_size || 13}px`, marginBottom: "4px" }}>{(settings.phone_numbers || []).join(" | ")}</div>
                <div style={{ width: "100%", textAlign: settings.email_align || "center", fontFamily: settings.email_font || "sans-serif", fontSize: `${settings.email_size || 13}px`, marginBottom: "4px" }}><a href={`mailto:${settings.email}`} style={{ color: settings.email_color || "#475569", textDecoration: 'none' }}>{settings.email}</a></div>
            ...
            <div className="meta-grid"><p><strong>{mode === "invoice" ? "Invoice No" : "Estimate No"}:</strong> {isNumberLoading ? "Generating..." : documentNumber || "-"}</p><p><strong>Date:</strong> {billDate}</p></div>
            ...
            <div className="sheet-bottom-stack">
              ...
              {computed.earnedPoints > 0 && (<div style={{ textAlign: "center", marginTop: "15px", padding: "10px", backgroundColor: "#f0fdf4", borderRadius: "8px", border: "1px dashed #22c55e", color: "#166534", fontWeight: "bold", fontSize: "0.9rem" }}>🎉 You earned {computed.earnedPoints} Loyalty Points on this bill!</div>)}
              <div className="declaration">
                {mode === "invoice" ? (<><p className="section-title">DECLARATION</p><p>We declare that this bill shows the actual price of items and all details are correct.</p><p className="section-title">POLICIES, T&C</p><ul className="policies-list"><li>6 Months of repair and polishing warranty only on silver ornaments.</li><li>You can replace purchased items within 7 days for manufacturing defects.</li></ul></>) : (<><p className="section-title">POLICIES, T&C</p><ul className="policies-list"><li>6 Months of repair and polishing warranty only on silver ornaments.</li><li>You can replace purchased items within 7 days for manufacturing defects.</li></ul></>)}
              </div>
        ...
        <aside className="controls no-print" style={{ flex: isMobileSplit ? "none" : "2", overflowY: isMobileSplit ? "visible" : "auto", overflowX: "hidden", padding: "20px", backgroundColor: "white", borderLeft: isMobileSplit ? "none" : "1px solid #cbd5e1", borderTop: isMobileSplit ? "1px solid #cbd5e1" : "none", height: isMobileSplit ? "max-content" : "100%" }}>
          <div className="control-card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
                ...
                <select value={billBranchId} onChange={async (e) => { const nextBranch = e.target.value; setBillBranchId(nextBranch); markDirty(); if (currentBillId) { try { const res = await axios.get(`${API}/bills/next-number?mode=${mode}&branch_id=${nextBranch}`, { headers: authHeaders }); setDocumentNumber(res.data.document_number); toast.info(`Migrating to Branch: ${nextBranch}`); } catch (err) { toast.error("Failed to fetch new number for migration."); } } else { await reserveNumber(mode, nextBranch); } }} style={{ padding: "4px 8px", borderRadius: "6px", border: "1px solid #cbd5e1", fontSize: "0.85rem", outline: "none", cursor: "pointer" }}>{(settings.branches || []).map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select>
            </div>
            <div style={{ marginBottom: "15px", paddingBottom: "15px", borderBottom: "1px dashed var(--border)" }}><label className="select-label" style={{ fontSize: "0.8rem", marginBottom: "4px", display: "block" }}>Bill Number (Editable)</label><Input value={documentNumber} onChange={(e) => { setDocumentNumber(e.target.value); markDirty(); }} placeholder="e.g. INV-0212" disabled={!!currentBillId} style={{ fontWeight: "bold", color: "var(--brand)", backgroundColor: currentBillId ? "#f1f5f9" : "white" }} /></div>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.85rem", marginBottom: "5px", color: "#16a34a", fontWeight: "bold" }}><span>{customer.name ? `Loyalty Points: ${customer.points} | Store Credit: ₹${money(customer.credit)}` : ""}</span></div>
            ...
            {(suggestions || []).length > 0 && (<div className="suggestions">{(suggestions || []).map((entry) => (<button key={entry.id} type="button" className="suggestion-item" onClick={() => { setCustomer({ name: entry.name, phone: entry.phone, address: entry.address, email: entry.email, points: entry.points || 0, credit: entry.credit || 0 }); setSuggestions([]); markDirty(); }}>{entry.name} · {entry.phone}</button>))}</div>)}
          </div>
          ...
          <div className="control-card">
            ...
            {upiAmountToPay > 0 && (<Button onClick={() => sendQrToDisplay(upiAmountToPay, upiId)} disabled={isMqttSending} style={{ width: "100%", marginBottom: "15px", backgroundColor: "#0f172a", color: "white", height: "50px", fontSize: "1rem", border: "2px solid #cbd5e1" }}>{isMqttSending ? "Processing..." : "🖥️ Show QR on Shop Display"}</Button>)}
            ...
            {txType === "sale" && (
              <div style={{ backgroundColor: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                ...
                <select id="paymentMethodSelect" value={paymentMethod} onChange={(e) => { setPaymentMethod(e.target.value); markDirty(); }} className="native-select"><option value="" disabled>Select Method</option><option value="Cash">Cash</option><option value="UPI">UPI</option>{mode === "invoice" && <option value="Card">Card</option>}<option value="Split">Split (Cash + UPI)</option></select>
                {paymentMethod === "Split" && (<div style={{ display: "flex", gap: "10px", marginTop: "10px" }}><Input value={splitCash} onChange={(e) => { setSplitCash(e.target.value); markDirty(); }} placeholder="Cash Received ₹" /><Input value={`UPI: ₹${money(Math.max(0, computed.grandTotal - num(splitCash)))}`} disabled style={{ backgroundColor: "#f1f5f9" }} /></div>)}
                <div style={{ marginTop: "15px", padding: "12px", backgroundColor: isPaymentDone ? "#dcfce7" : "#fef3c7", border: `1.5px solid ${isPaymentDone ? "#22c55e" : "#f59e0b"}`, borderRadius: "8px", display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }} onClick={() => { const next = !isPaymentDone; setIsPaymentDone(next); markDirty(); if (next && iotOnline) sendSuccessToDisplay(); }}>
                  <input type="checkbox" checked={isPaymentDone} onChange={(e) => { const next = e.target.checked; setIsPaymentDone(next); markDirty(); if (next && iotOnline) sendSuccessToDisplay(); }} onClick={(e) => e.stopPropagation()} style={{ width: "20px", height: "20px", cursor: "pointer" }} />
            ...
            {(txType === "booking" || txType === "service") && (
              <div style={{ backgroundColor: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #cbd5e1' }}>
                ...
                <select value={advanceMethod} onChange={(e) => { setAdvanceMethod(e.target.value); markDirty(); }} className="native-select"><option value="" disabled>Select Advance Method</option><option value="Cash">Cash</option><option value="UPI">UPI</option>{mode === "invoice" && <option value="Card">Card</option>}<option value="Split">Split (Cash + UPI)</option></select>
                {advanceMethod === "Split" && (<div style={{ display: "flex", gap: "10px", marginTop: "10px" }}><Input value={advanceSplitCash} onChange={(e) => { setAdvanceSplitCash(e.target.value); markDirty(); }} placeholder="Cash Portion ₹" /><Input value={`UPI: ₹${money(Math.max(0, num(advanceAmount) - num(advanceSplitCash)))}`} disabled style={{ backgroundColor: "#f1f5f9" }} /></div>)}
                <div style={{ marginTop: "15px", padding: "10px", backgroundColor: isAdvancePaid ? "#dcfce7" : "#fef3c7", border: `1px solid ${isAdvancePaid ? "#22c55e" : "#f59e0b"}`, borderRadius: "8px", display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }} onClick={() => { const next = !isAdvancePaid; setIsAdvancePaid(next); markDirty(); if (next && iotOnline) sendSuccessToDisplay(); }}>
                  <input type="checkbox" checked={isAdvancePaid} onChange={(e) => { const next = e.target.checked; setIsAdvancePaid(next); markDirty(); if (next && iotOnline) sendSuccessToDisplay(); }} onClick={(e) => e.stopPropagation()} style={{ width: "16px", height: "16px" }} />
                ...
                <select value={balanceMethod} onChange={(e) => { setBalanceMethod(e.target.value); markDirty(); }} className="native-select"><option value="" disabled>Select Balance Method</option><option value="Cash">Cash</option><option value="UPI">UPI</option>{mode === "invoice" && <option value="Card">Card</option>}<option value="Split">Split (Cash + UPI)</option></select>
                {balanceMethod === "Split" && (<div style={{ display: "flex", gap: "10px", marginTop: "10px" }}><Input value={balanceSplitCash} onChange={(e) => { setBalanceSplitCash(e.target.value); markDirty(); }} placeholder="Cash Portion ₹" /><Input value={`UPI: ₹${money(Math.max(0, (computed.grandTotal - num(advanceAmount)) - num(balanceSplitCash)))}`} disabled style={{ backgroundColor: "#f1f5f9" }} /></div>)}
                <div style={{ marginTop: "15px", padding: "10px", backgroundColor: isBalancePaid ? "#dcfce7" : "#fef3c7", border: `1px solid ${isBalancePaid ? "#22c55e" : "#f59e0b"}`, borderRadius: "8px", display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }} onClick={() => { const next = !isBalancePaid; setIsBalancePaid(next); markDirty(); if (next && iotOnline) sendSuccessToDisplay(); }}>
                  <input type="checkbox" checked={isBalancePaid} onChange={(e) => { const next = e.target.checked; setIsBalancePaid(next); markDirty(); if (next && iotOnline) sendSuccessToDisplay(); }} onClick={(e) => e.stopPropagation()} style={{ width: "16px", height: "16px" }} />
        ...
        <section className="side-drawer no-print" style={{ position: "fixed", top: 0, bottom: 0, right: 0, width: "100vw", maxWidth: "650px", backgroundColor: "white", zIndex: 100, boxShadow: "-5px 0 25px rgba(0,0,0,0.2)", overflowY: "auto" }}>
          ...
          <div style={{ padding: "20px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
               ...
               <Button variant={showGraph ? "default" : "outline"} onClick={() => setShowGraph(!showGraph)} style={{ backgroundColor: showGraph ? "#0f172a" : "white", color: showGraph ? "white" : "black" }}>📊 {showGraph ? "Hide Graph" : "Show Analytics Graph"}</Button>
            </div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "15px" }}>
                    <div style={{ flex: "2 1 300px", backgroundColor: "white", border: "1px solid #e2e8f0", padding: "15px", borderRadius: "12px", height: "250px" }}>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={graphData?.barData || []}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" fontSize={10} /><YAxis hide /><RechartsTooltip formatter={(value) => `₹${money(value)}`} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }} /><Bar dataKey="amount" fill="#16a34a" radius={[4, 4, 0, 0]} /></BarChart>
                      </ResponsiveContainer>
                    </div>
            ...
            <div style={{ marginBottom: "25px" }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
                <div style={{ flex: "1 1 140px", backgroundColor: "#fffbeb", border: "1px solid #fde68a", padding: "15px", borderRadius: "10px", textAlign: "center" }}>
                  <Banknote size={24} color="#d97706" style={{ margin: "0 auto 8px auto" }} /><p style={{ margin: "0 0 5px 0", fontSize: "0.85rem", color: "#92400e", fontWeight: "bold" }}>Cash Drawer</p>
                ...
                <div style={{ flex: "1 1 140px", backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", padding: "15px", borderRadius: "10px", textAlign: "center" }}>
                  <Wallet size={24} color="#2563eb" style={{ margin: "0 auto 8px auto" }} /><p style={{ margin: "0 0 5px 0", fontSize: "0.85rem", color: "#1e40af", fontWeight: "bold" }}>Estimate Bank</p>
                ...
                <div style={{ flex: "1 1 140px", backgroundColor: "#fef2f2", border: "1px solid #fecaca", padding: "15px", borderRadius: "10px", textAlign: "center" }}>
                  <Building2 size={24} color="#dc2626" style={{ margin: "0 auto 8px auto" }} /><p style={{ margin: "0 0 5px 0", fontSize: "0.85rem", color: "#991b1b", fontWeight: "bold" }}>GST Bank</p>
              ...
              <div style={{ textAlign: "right", marginTop: "10px" }}>
                 {!editingBalances ? (<Button size="sm" variant="outline" onClick={() => { setManualCash(activeGlobalBranch?.cash_balance || 0); setManualEstBank(activeGlobalBranch?.estimate_bank_balance || 0); setManualInvBank(activeGlobalBranch?.invoice_bank_balance || 0); setEditingBalances(true); }}>Manually Edit Balances</Button>) : (<div style={{ display: "inline-flex", gap: "10px" }}><Button size="sm" variant="outline" onClick={() => setEditingBalances(false)}>Cancel</Button><Button size="sm" style={{ backgroundColor: "#16a34a" }} onClick={saveBalances}>Save Balances</Button></div>)}
              </div>
            </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  {(todayBills || []).map(b => (
                    <div key={b.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px", border: "1px solid #e2e8f0", borderRadius: "6px", backgroundColor: "white" }}>
                      <div>
                        <div style={{ display: "flex", gap: "8px", alignItems: "center", marginBottom: "4px" }}><strong style={{ color: b.mode === 'invoice' ? "#dc2626" : "#2563eb" }}>{b.document_number}</strong><span style={{ fontSize: "0.75rem", padding: "2px 6px", backgroundColor: (b.tx_type === "sale" ? b.is_payment_done : b.is_balance_paid) ? "#dcfce7" : "#fee2e2", color: (b.tx_type === "sale" ? b.is_payment_done : b.is_balance_paid) ? "#166534" : "#991b1b", borderRadius: "4px" }}>{(b.tx_type === "sale" ? b.is_payment_done : b.is_balance_paid) ? "Paid" : "Pending"}</span></div>
                        <div style={{ fontSize: "0.85rem", color: "#475569" }}>{b.customer_name || b.customer?.name || "Unknown"} • {b.tx_type || "Sale"}</div>
                      </div>
                      <div style={{ textAlign: "right" }}><strong style={{ fontSize: "1.1rem" }}>₹{money(b.totals?.grand_total || 0)}</strong></div>
                    </div>
                  ))}
                </div>
              )}
          ...
          <div style={{ padding: "15px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "20px" }}>
              ...
              <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
                <select value={recentBranchFilter} onChange={(e) => setRecentBranchFilter(e.target.value)} className="native-select" style={{ flex: 1, minWidth: "120px" }}><option value="ALL">All Branches</option>{(settings.branches || []).map(b => <option key={b.id} value={b.id}>{b.name}</option>)}</select>
                <select value={recentModeFilter} onChange={(e) => setRecentModeFilter(e.target.value)} className="native-select" style={{ flex: 1, minWidth: "120px" }}><option value="ALL">All Modes</option><option value="invoice">Invoices</option><option value="estimate">Estimates</option></select>
                <select value={recentDateFilter} onChange={(e) => setRecentDateFilter(e.target.value)} className="native-select" style={{ flex: 1, minWidth: "120px" }}><option value="ALL">All Time</option><option value="THIS_MONTH">This Month</option><option value="LAST_MONTH">Last Month</option><option value="CUSTOM">Custom Range</option></select>
              </div>
              {recentDateFilter === "CUSTOM" && (<div style={{ display: "flex", gap: "10px" }}><Input type="date" value={customStartDate} onChange={(e) => setCustomStartDate(e.target.value)} style={{ flex: 1 }} /><Input type="date" value={customEndDate} onChange={(e) => setCustomEndDate(e.target.value)} style={{ flex: 1 }} /></div>)}
            </div>

            {loadingRecent ? (<p style={{ textAlign: "center", padding: "20px" }}>Loading recent bills...</p>) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {(Array.isArray(filteredRecentBills) ? filteredRecentBills : []).length === 0 ? (<p style={{ textAlign: "center", color: "#64748b", padding: "20px" }}>No bills found matching criteria.</p>) : (
                  (Array.isArray(filteredRecentBills) ? filteredRecentBills : []).map((bill) => (
                    <div key={bill.id} className="recent-bill-card" style={{ padding: "15px", border: "1px solid #cbd5e1", borderRadius: "8px", backgroundColor: "#f8fafc" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}><strong style={{ fontSize: "1.1rem", color: bill.mode === "invoice" ? "#dc2626" : "#2563eb" }}>{bill.document_number}</strong><span style={{ fontSize: "0.85rem", color: "#475569" }}>{bill.date}</span></div>
                      <p style={{ margin: "0 0 5px 0", fontWeight: "bold" }}>{bill.customer_name || bill.customer?.name || "Unknown Customer"}</p>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px", fontSize: "0.9rem" }}><span>Total: <strong>₹{money(bill.totals?.grand_total || 0)}</strong></span><span style={{ padding: "3px 8px", borderRadius: "12px", fontSize: "0.75rem", backgroundColor: (bill.tx_type === "sale" ? bill.is_payment_done : bill.is_balance_paid) ? "#dcfce7" : "#fef3c7", color: (bill.tx_type === "sale" ? bill.is_payment_done : bill.is_balance_paid) ? "#166534" : "#b45309" }}>{(bill.tx_type === "sale" ? bill.is_payment_done : bill.is_balance_paid) ? "PAID" : "PENDING"}</span></div>
                      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}><Button size="sm" onClick={() => loadBillForEditing(bill)} style={{ flex: 1, backgroundColor: "#0f172a" }}>Edit</Button><Button size="sm" variant="outline" onClick={() => downloadPdf(`bulk-bill-${bill.document_number}`, bill.document_number)} style={{ flex: 1 }}>PDF</Button><Button size="sm" variant="outline" onClick={() => handleQuickPaymentToggle(bill)} style={{ flex: 1 }}>{bill.is_payment_done ? "Mark Pending" : "Mark Paid"}</Button><Button size="sm" variant="outline" onClick={() => handleDeleteBill(bill)} style={{ borderColor: "#ef4444", color: "#ef4444" }}>Del</Button></div>
                    </div>
                  ))
                )}
              </div>
            ...
            {settingsTab === "design" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                <div style={{ padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
                  <h4>Print Scale Alignment</h4><p style={{ fontSize: "0.85rem", color: "#64748b", marginBottom: "10px" }}>Adjust this if your printed bill cuts off or is too small. (Default 100%)</p>
                  <div style={{ display: "flex", alignItems: "center", gap: "15px" }}><input type="range" min="98" max="102" step="0.5" value={printScale} onChange={(e) => setPrintScale(Number(e.target.value))} style={{ flex: 1 }} /><strong style={{ minWidth: "50px", textAlign: "right" }}>{printScale}%</strong></div>
                </div>
                <div style={{ padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #e2e8f0" }}><h4>Custom Fonts</h4><p style={{ fontSize: "0.85rem", color: "#64748b", marginBottom: "10px" }}>Upload a .ttf or .woff file to use custom fonts.</p><Input type="file" accept=".ttf,.woff,.woff2" onChange={handleFontUpload} /></div>
                <div style={{ padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #e2e8f0" }}><h4>Logos</h4><label className="select-label">Shop Logo (PNG/JPG)</label><Input type="file" accept="image/*" onChange={handleLogoUpload} style={{ marginBottom: "10px" }} />{logoUploadName && <p style={{ fontSize: "0.8rem", color: "#16a34a" }}>Selected: {logoUploadName}</p>}</div>
            ...
            {settingsTab === "business" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                <div style={{ padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #cbd5e1" }}>
                  ...
                  <label className="select-label">Today's Silver Rate (₹ per gram)</label><Input type="number" value={settings.silver_rate_per_gram} onChange={(e) => setSettings({ ...settings, silver_rate_per_gram: e.target.value })} style={{ marginBottom: "10px" }} />
                  <label className="select-label">Default Making Charge (₹ per gram)</label><Input type="number" value={settings.making_charge_per_gram} onChange={(e) => setSettings({ ...settings, making_charge_per_gram: e.target.value })} style={{ marginBottom: "10px" }} />
                  <label className="select-label">Flat MC for Items Below 5g (₹)</label><Input type="number" value={settings.flat_mc_below_5g} onChange={(e) => setSettings({ ...settings, flat_mc_below_5g: e.target.value })} style={{ marginBottom: "10px" }} />
                  <label className="select-label">Default HSN Code</label><Input value={settings.default_hsn} onChange={(e) => setSettings({ ...settings, default_hsn: e.target.value })} />
                </div>
                <div style={{ padding: "15px", backgroundColor: "#f0fdf4", borderRadius: "8px", border: "1px solid #bbf7d0" }}>
                  ...
                  <label className="select-label">Points Earned Per 1 Gram</label><Input type="number" value={settings.loyalty_points_per_gram !== undefined ? settings.loyalty_points_per_gram : 1} onChange={(e) => setSettings({ ...settings, loyalty_points_per_gram: e.target.value })} style={{ marginBottom: "10px" }} />
                  <label className="select-label">Rupees (₹) Discount Per 1 Point</label><Input type="number" value={settings.loyalty_point_value_rs !== undefined ? settings.loyalty_point_value_rs : 1} onChange={(e) => setSettings({ ...settings, loyalty_point_value_rs: e.target.value })} style={{ marginBottom: "10px" }} />
                </div>
                <div style={{ padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #cbd5e1" }}>
                  <h4 style={{ margin: "0 0 15px 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>Branch Management<Button size="sm" onClick={addBranch} style={{ backgroundColor: "#0f172a" }}>+ Add Branch</Button></h4>
                  {(settings.branches || []).map((branch, index) => (
                    <div key={branch.id} style={{ marginBottom: "20px", padding: "15px", border: "1px dashed #cbd5e1", borderRadius: "8px", backgroundColor: "white" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}><strong>Branch {index + 1}</strong><Button size="sm" variant="outline" onClick={() => removeBranch(index)} style={{ borderColor: "#ef4444", color: "#ef4444" }}>Remove</Button></div>
                      <label className="select-label">Branch Name</label><Input value={branch.name} onChange={(e) => updateBranch(index, 'name', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">Branch Address</label><Input value={branch.address} onChange={(e) => updateBranch(index, 'address', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">Google Maps Location URL (For Address Click)</label><Input value={branch.location_url || ""} onChange={(e) => updateBranch(index, 'location_url', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">Google Maps Feedback/Review URL (For ⭐ Button & QR)</label><Input value={branch.map_url || ""} onChange={(e) => updateBranch(index, 'map_url', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">WhatsApp Group/Chat URL (For Button & QR)</label><Input value={branch.whatsapp_url || ""} onChange={(e) => updateBranch(index, 'whatsapp_url', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">Instagram URL (For Button & QR)</label><Input value={branch.instagram_url || ""} onChange={(e) => updateBranch(index, 'instagram_url', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">About Us URL (For Print QR)</label><Input value={branch.about_url || ""} onChange={(e) => updateBranch(index, 'about_url', e.target.value)} style={{ marginBottom: "10px" }} />
                      <label className="select-label">GSTIN Number (Optional)</label><Input value={branch.gstin || ""} onChange={(e) => updateBranch(index, 'gstin', e.target.value)} placeholder="Leave blank if no GST" style={{ marginBottom: "10px" }} />
                      <div style={{ display: "flex", gap: "10px", marginTop: "5px" }}><div style={{ flex: 1 }}><label className="select-label">Invoice UPI ID</label><Input value={branch.invoice_upi_id} onChange={(e) => updateBranch(index, 'invoice_upi_id', e.target.value)} placeholder="name@bank" /></div><div style={{ flex: 1 }}><label className="select-label">Estimate UPI ID</label><Input value={branch.estimate_upi_id} onChange={(e) => updateBranch(index, 'estimate_upi_id', e.target.value)} placeholder="name@bank" /></div></div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {settingsTab === "advanced" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                <div style={{ padding: "15px", backgroundColor: "#f0f9ff", borderRadius: "8px", border: "1px solid #bae6fd" }}>
                  <h4 style={{ color: "#0369a1", margin: "0 0 10px 0", display: "flex", alignItems: "center", gap: "8px" }}><Cpu size={18} /> Counter Display Terminal</h4>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px", backgroundColor: "white", borderRadius: "6px", border: "1px solid #e0f2fe" }}>
                    <div style={{ position: "relative" }}><span style={{ display: "block", width: "12px", height: "12px", borderRadius: "50%", backgroundColor: iotOnline ? "#22c55e" : "#ef4444" }} />{iotOnline && <span style={{ position: "absolute", top: 0, left: 0, width: "12px", height: "12px", borderRadius: "50%", backgroundColor: "#22c55e", animation: "ping 2s cubic-bezier(0, 0, 0.2, 1) infinite" }} />}</div>
                    <div><p style={{ margin: 0, fontWeight: "bold", color: "#0c4a6e" }}>{iotOnline ? "DISPLAY ONLINE" : "DISPLAY OFFLINE"}</p><p style={{ margin: 0, fontSize: "0.75rem", color: "#64748b" }}>{iotOnline ? "Ready to show QR codes and payment animations." : "Check if the shop ESP32 device is powered on."}</p></div>
                  </div>
                  <p style={{ fontSize: "0.75rem", color: "#64748b", marginTop: "10px" }}>This device is synchronized with your cloud billing app to automate customer payments.</p>
                </div>
                ...
                <div style={{ padding: "15px", backgroundColor: "#f0fdfa", borderRadius: "8px", border: "1px solid #ccfbf1" }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px", fontSize: "0.85rem", color: "#0f766e" }}><span>Storage Used: {((storageStats?.used_bytes || 0) / 1024 / 1024).toFixed(2)} MB</span><span>Limit: {((storageStats?.quota_bytes || 524288000) / 1024 / 1024).toFixed(2)} MB</span></div>
                  <div style={{ width: "100%", height: "8px", backgroundColor: "#ccfbf1", borderRadius: "4px", marginBottom: "15px", overflow: "hidden" }}><div style={{ height: "100%", width: `${storageStats?.percentage || 0}%`, backgroundColor: (storageStats?.percentage || 0) > 80 ? "#ef4444" : "#14b8a6" }}></div></div>
                ...
                <div style={{ padding: "15px", backgroundColor: "#f8fafc", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
                  <h4 style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "0 0 15px 0" }}><div style={{ display: "flex", alignItems: "center", gap: "8px" }}><Keyboard size={18} /> Keyboard Shortcuts</div><Button size="sm" onClick={addShortcut} style={{ backgroundColor: "#0f172a" }}><Plus size={16} style={{ marginRight: "5px" }} /> Add Shortcut</Button></h4>
                  <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
