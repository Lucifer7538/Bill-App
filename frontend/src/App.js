  ...
  useEffect(() => {
    if (showRecentBills && token && !isPublicView) {
      const fetchRecent = async () => {
        ...
        try { const limit = recentDateFilter === "ALL" ? 50 : 500; const response = await axios.get(`${API}/bills/recent?limit=${limit}&branch_filter=${recentBranchFilter}&search=${encodeURIComponent(billSearchQuery)}`, { headers: authHeaders }); setRecentBillsList(Array.isArray(response.data) ? response.data : (Array.isArray(response.data?.data) ? response.data.data : [])); } 
...
// === PART 4 START ===
  const loadSettings = async () => {
    ...
    if (dbData.shortcuts && dbData.shortcuts.length > 0) { const customOnly = dbData.shortcuts.filter(sc => !sc.isSystem); const systemUpdated = defaultSettings.shortcuts.map(sys => { const savedSys = dbData.shortcuts.find(s => s.id === sys.id); return savedSys ? savedSys : sys; }); mergedShortcuts = [...systemUpdated, ...customOnly]; } else { mergedShortcuts = defaultSettings.shortcuts; }
  ...
  const handleNewBillClick = async () => { if (currentBillId && isDirty) { if (!window.confirm("⚠️ You have unsaved edits! Discard and start new?")) return; } else if (!currentBillId && !checkIsBlank()) { if (!window.confirm("⚠️ Discard entered data and start blank bill?")) return; } await clearBill(mode, billBranchId); };
  ...
  const sendQrToDisplay = async (amount, upiId) => {
    ...
    try { await axios.post(`${API}/cloud/mqtt/publish`, { topic: "Jalaram/QR", message: JSON.stringify({ amount: String(Math.round(amount)), upi_id: upiId }) }, { headers: authHeaders }); toast.success("QR Code sent to shop display!"); } 
    catch (e) { toast.error("Failed to send QR to display."); } finally { setIsMqttSending(false); }
  };

  const sendSuccessToDisplay = async () => { try { await axios.post(`${API}/cloud/mqtt/publish`, { topic: "Jalaram/QR", message: "SUCCESS" }, { headers: authHeaders }); } catch (e) { console.error("MQTT Success trigger failed"); } };
  ...
  const shareWhatsApp = () => { 
    ...
    // CRASH GUARD: Forces phone to a string before running `.replace()`
...
// === PART 4 END ===
// === PART 5 START ===
  // // EARLY RETURNS (LOGIN GUARDS & PUBLIC VIEWERS)
  // These must happen before the main dashboard returns
  if (isPublicView) {
      ...
      <main className="main-layout" style={isPrinting ? { height: "auto", overflow: "visible", display: "block" } : { flex: 1, display: "flex", flexDirection: isMobileSplit ? "column" : "row", overflowY: isMobileSplit ? "auto" : "hidden", overflowX: "hidden", backgroundColor: "#f1f5f9", minHeight: 0, paddingBottom: isMobileSplit ? "40px" : "0" }}>
          ...
          <div id="bill-print-root" className="bill-sheet" style={{ "--print-scale-factor": (printScale / 100).toFixed(3), position: 'relative', zIndex: 1, margin: "0 auto" }}>
            ...
            <div className="sheet-bottom-stack">
              <div className="totals">
                ...
                {num(computed.redeemedPoints) > 0 && <div className="totals-row"><span style={{color:"#16a34a"}}>POINTS REDEEMED ({computed.redeemedPoints} pts)</span><strong style={{color:"#16a34a"}}>- ₹{money(computed.redeemedValue)}</strong></div>}
                {num(computed.appliedCredit) > 0 && <div className="totals-row"><span style={{color:"#16a34a"}}>STORE CREDIT APPLIED</span><strong style={{color:"#16a34a"}}>- ₹{money(computed.appliedCredit)}</strong></div>}
                ...
                {num(computed.savedCredit) > 0 && <div className="totals-row"><span>STORE CREDIT SAVED</span><strong>+ ₹{money(computed.savedCredit)}</strong></div>}
              ...
              <div className="declaration">{mode === "invoice" ? (<><p className="section-title">DECLARATION</p><p>We declare that this bill shows the actual price of items and all details are correct.</p><p className="section-title">POLICIES, T&C</p><ul className="policies-list"><li>6 Months of repair and polishing warranty only on silver ornaments.</li><li>You can replace purchased items within 7 days for manufacturing defects.</li></ul></>) : (<><p className="section-title">POLICIES, T&C</p><ul className="policies-list"><li>6 Months of repair and polishing warranty only on silver ornaments.</li><li>You can replace purchased items within 7 days for manufacturing defects.</li></ul></>)}</div>
        ...
        <aside className="controls no-print" style={{ flex: isMobileSplit ? "none" : "2", overflowY: isMobileSplit ? "visible" : "auto", overflowX: "hidden", padding: "20px", backgroundColor: "white", borderLeft: isMobileSplit ? "none" : "1px solid #cbd5e1", borderTop: isMobileSplit ? "1px solid #cbd5e1" : "none", height: isMobileSplit ? "max-content" : "100%" }}>
          ...
          <div className="control-card">
            <h3>Item Lines</h3>
            {(items || []).map((item) => {
              // --- THE RED DOT ENGINE (CRASH PROOF) ---
              ...
              <div key={item.id} className="item-row-editor" style={{ overflow: "visible" }}>
                <div style={{ position: "relative" }}>
                  <Input 
                    ...
                    onBlur={() => { if (hasTypo) updateItem(item.id, "description", correctedText); }}
                    placeholder="Description" style={{ width: "100%", paddingRight: hasTypo ? "110px" : "10px" }}
                  />
                  {hasTypo && (
                    <button 
                      type="button" onMouseDown={(e) => { e.preventDefault(); updateItem(item.id, 'description', correctedText); }}
        ...
        <section className="side-drawer no-print" style={{ position: "fixed", top: 0, bottom: 0, right: 0, width: "100vw", maxWidth: "650px", backgroundColor: "white", zIndex: 100, boxShadow: "-5px 0 25px rgba(0,0,0,0.2)", overflowY: "auto" }}>
          ...
          <div style={{ padding: "20px" }}>
              ...
              <div style={{ marginBottom: "30px", padding: "15px", backgroundColor: "#f8fafc", border: "1px solid #cbd5e1", borderRadius: "10px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px", flexWrap: "wrap", gap: "10px" }}>
                  ...
                  <div style={{ backgroundColor: "#0f172a", color: "white", padding: "4px 10px", borderRadius: "12px", fontSize: "0.85rem", fontWeight: "bold" }}>
                     Weight Sold: {num(graphData?.totalWeight || 0).toFixed(3)} g
                  </div>
                </div>
                    ...
                    <div style={{ flex: "1 1 200px", backgroundColor: "white", border: "1px solid #e2e8f0", padding: "15px", borderRadius: "12px", height: "250px", textAlign: "center" }}>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '5px' }}>
                         {(graphData?.pieData || []).map((entry, i) => (
                           <span key={i} style={{ fontSize: '0.65rem', fontWeight: 'bold', color: COLORS[i % COLORS.length] }}>● {entry.name}</span>
                         ))}
                      </div>
                    </div>
                  </div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: "10px" }}>
                <div style={{ flex: "1 1 140px", backgroundColor: "#fffbeb", border: "1px solid #fde68a", padding: "15px", borderRadius: "10px", textAlign: "center" }}>
                  ...
                  {editingBalances ? (
                    ...
                    <h3 style={{ margin: 0, color: "#b45309", fontSize: "1.3rem" }}>₹{money(activeGlobalBranch?.cash_balance || 0)}</h3>
                  )}
                </div>
                <div style={{ flex: "1 1 140px", backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", padding: "15px", borderRadius: "10px", textAlign: "center" }}>
                  ...
                  {editingBalances ? (
                    ...
                    <h3 style={{ margin: 0, color: "#1d4ed8", fontSize: "1.3rem" }}>₹{money(activeGlobalBranch?.estimate_bank_balance || 0)}</h3>
                  )}
                </div>
                <div style={{ flex: "1 1 140px", backgroundColor: "#fef2f2", border: "1px solid #fecaca", padding: "15px", borderRadius: "10px", textAlign: "center" }}>
                  ...
                  {editingBalances ? (
                    ...
                    <h3 style={{ margin: 0, color: "#b91c1c", fontSize: "1.3rem" }}>₹{money(activeGlobalBranch?.invoice_bank_balance || 0)}</h3>
                  )}
                </div>
              </div>
            ...
            <div style={{ marginBottom: "30px" }}>
              ...
              {(Array.isArray(ledgerLogs) ? ledgerLogs : []).length === 0 ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {(Array.isArray(ledgerLogs) ? ledgerLogs : []).map(log => (
            ...
            <div>
              ...
              {ledgerLoading ? (
                <p>Loading bills...</p>
              ) : (Array.isArray(todayBills) ? todayBills : []).length === 0 ? (
                <p style={{ color: "#666" }}>No bills generated today yet.</p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                  {(Array.isArray(todayBills) ? todayBills : []).map(b => (
      ...
      {showRecentBills && (
        <section className="side-drawer no-print" style={{ position: "fixed", top: 0, bottom: 0, right: 0, width: "100vw", maxWidth: "550px", backgroundColor: "white", zIndex: 100, boxShadow: "-5px 0 25px rgba(0,0,0,0.2)", overflowY: "auto" }}>
          <div className="drawer-header" style={{ position: "sticky", top: 0, backgroundColor: "white", zIndex: 10, paddingBottom: "15px", borderBottom: "1px solid #e2e8f0" }}>
            ...
            <Button type="button" variant="outline" className="drawer-back-btn" onClick={() => setShowRecentBills(false)} style={{ marginLeft: "20px" }}>
              <ArrowLeft className="drawer-back-icon" style={{ marginRight: "5px" }} />
          ...
          <div style={{ padding: "15px" }}>
            <div style={{ marginBottom: "20px" }}>
              <Button onClick={handleBulkDownload} disabled={isBulkDownloading || (Array.isArray(filteredRecentBills) ? filteredRecentBills : []).length === 0} style={{ width: "100%", backgroundColor: "#0f172a", height: "auto", padding: "10px", display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center", justifyContent: "center", fontSize: "1rem", boxSizing: "border-box" }}>
                {isBulkDownloading ? "Generating PDF... Please Wait" : <><Download size={18} /> Download {(Array.isArray(filteredRecentBills) ? filteredRecentBills : []).length} Bills as PDF</>}
              </Button>
            </div>
            ...
            {loadingRecent ? (
              <p style={{ textAlign: "center", padding: "20px" }}>Loading recent bills...</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                {(Array.isArray(filteredRecentBills) ? filteredRecentBills : []).length === 0 ? (
                  <p style={{ textAlign: "center", color: "#64748b", padding: "20px" }}>No bills found matching criteria.</p>
                ) : (
      ...
      {showSettings && (
        <section className="side-drawer no-print" style={{ position: "fixed", top: 0, bottom: 0, right: 0, width: "100vw", maxWidth: "600px", backgroundColor: "white", zIndex: 100, boxShadow: "-5px 0 25px rgba(0,0,0,0.2)", overflowY: "auto" }}>
          <div className="drawer-header" style={{ position: "sticky", top: 0, backgroundColor: "white", zIndex: 10, paddingBottom: "15px", borderBottom: "1px solid #e2e8f0" }}>
            ...
            <Button type="button" variant="outline" className="drawer-back-btn" onClick={() => setShowSettings(false)} style={{ marginLeft: "20px" }}>
              <ArrowLeft className="drawer-back-icon" style={{ marginRight: "5px" }} />
              <span>Close Menu</span>
            </Button>
          </div>
          <div style={{ padding: "15px" }}>
            ...
            {settingsTab === "advanced" && (
              <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                ...
                <div style={{ padding: "15px", backgroundColor: "#fef2f2", borderRadius: "8px", border: "1px solid #fecaca" }}>
                  ...
                  <Button onClick={() => handleResetCounter("estimate")} style={{ width: "100%", marginBottom: "10px", backgroundColor: "#b91c1c", color: "white" }}>
                    Reset Estimate Counter
                  </Button>
                  <Button onClick={() => handleResetCounter("invoice")} style={{ width: "100%", marginBottom: "10px", backgroundColor: "#b91c1c", color: "white" }}>
                    Reset Invoice Counter
                  </Button>
                  <Button onClick={handleDeleteAllBills} style={{ width: "100%", backgroundColor: "#7f1d1d", color: "white" }}>
                    WIPE ALL BILLS
                  </Button>
                </div>
                ...
                <div style={{ padding: "15px", backgroundColor: "#f0fdfa", borderRadius: "8px", border: "1px solid #ccfbf1" }}>
                  ...
                  <Button onClick={handleBackupBills} style={{ width: "100%", backgroundColor: "#0f766e", color: "white" }}>
                    <Download size={16} style={{ marginRight: "8px" }} /> Download JSON Backup
                  </Button>
                </div>
            ...
            <Button onClick={saveSettings} style={{ width: "100%", marginTop: "20px", backgroundColor: "#0f172a", padding: "15px", fontSize: "1.1rem" }}>
              Save All Settings
            </Button>
          </div>
        </section>
